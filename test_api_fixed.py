"""
test_api.py - Complete API test for all endpoints
...
def test_49d():
    ...
    print("=" * 50)
    resp = requests.post(f"{BASE_URL}/api/49d/stats", json={"text": "Rama"})
    print(f"Status: {resp.status_code}")
...
def test_sandhi():
    ...
    print("=" * 50)
    resp = requests.post(f"{BASE_URL}/api/sandhi", json={"left": "deva?", "right": "api"})
    print(f"Status: {resp.status_code}")
    print(f"Result: {resp.json()['result']}")
    print(f"Rule: {resp.json()['rule_applied']}\n")
    return resp.ok
...
def test_nakshatra():
    ...
    print("=" * 50)
    resp = requests.post(f"{BASE_URL}/api/nakshatra/analyze", json={"text": "Rama"})
    if resp.ok:
        data = resp.json()
...
def test_prosody():
    ...
    print("=" * 50)
    resp = requests.post(f"{BASE_URL}/api/prosody/validate", json={"text": "rama? raja"})
    if resp.ok:
        data = resp.json()
...
def test_anumana():
    ...
    if resp.ok:
        data = resp.json()
        print(f"Current Rasa: {data['current_state']['rasa']}")
        print(f"Current Nakshatra: {data['current_state']['nakshatra']}")
        print(f"? Predicted Rasa: {data['predicted_state']['rasa']}")
        print(f"Transition: {data['transition_type']}")
        print(f"Confidence: {data['confidence']}")
        print(f"Suggested Raga: {data['suggested_raga']}")
        print(f"Suggested Meter: {data['suggested_meter']}")
        print(f"Prakriti: {data['prakriti']}\n")
    return resp.ok
...
def test_dataset():
    ...
    if resp.ok:
        data = resp.json()
        print(f"Random Nakshatra: {data['nakshatra']}")
        print(f"Raga: {data['raga']}")
        print(f"Seed Word: {data['seed_word']}\n")
    return resp.ok
